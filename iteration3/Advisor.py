...
class Advisor(Lecturer, IDisplayMenu):
    ...
    def printRequests(self):
        try:
            ...
            print("A type error occurred. Perhaps an incorrect data type was used as input. Please check your inputs.")

        except Exception as e:
            logging.error(f"An unknown error occurred while printing requests: {str(e)}")
    ...
    def checkAndPrintOverlap(self, request):
        ...
        for i in range(len(nested_list) - 1):
            conflict = 0
            if i in processedCourses:
                continue
            controlInput = 0
            for j in range(i + 1, len(nested_list)):
                if j in processedCourses:
                    continue
    ...
    def replyRequests(self):
        try:
            ...
            logging.error(f"An unknown error occurred while replying to requests: {str(e)}")
            print("An unknown error occurred, please try again later.")
